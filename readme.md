...
4. Публикация локальных репозиториев в GitLab (`publish`)
...
Создаёт проекты в GitLab и пушит все ветки и теги из локальных рабочих копий.

```bash
# одна рабочая копия → личный проект (имя = имя каталога)
$ gitlab-tools publish .

# одна рабочая копия → в группу
$ gitlab-tools publish . --root-group my-group

# несколько репозиториев под общим корнем → в группу с воссозданием структуры подкаталогов
$ gitlab-tools publish ./monorepo --root-group my-group

# пропустить уже опубликованные (remote origin уже есть)
$ gitlab-tools publish ./monorepo --root-group my-group --existing skip
```

| Режим | `--root-group` | Куда создаётся репозиторий |
|-------|----------------|----------------------------|
| `<dir>` — рабочая копия | необязателен | без опции — личные проекты; с опцией — в указанную группу |
| `<dir>` — корень нескольких реп | **обязателен** | в `--root-group`, структура подкаталогов → вложенные группы |

Опция `--existing` (по умолчанию `replace`): `skip` — пропустить рабочую копию, если remote с `--remote-name` уже есть; `rename` / `replace` — переименовать или заменить remote, если он указывает на другой хост, чем в конфиге.
